    ...
    return (
        <div className="bg-slate-900 text-white p-6 rounded-2xl shadow-xl ring-1 ring-white/10 relative z-50">
            <div className="flex items-center gap-3 mb-4">
                <div className="w-10 h-10 bg-stone-500 rounded-xl flex items-center justify-center shrink-0">
                    <MapPin className="text-slate-950" size={20} />
                </div>
                <div>
                    <h3 className="font-bold text-lg">Installateur Chauffage / PAC Local</h3>
                    <p className="text-stone-200 text-xs">Trouvez votre expert de proximité</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSearch} className="mb-4 relative">
                <div className="relative">
                    <input
                        ...
                        className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 pl-10 text-sm focus:ring-2 focus:ring-stone-500 outline-none transition placeholder:text-slate-500 text-white"
                        autoComplete="off"
                    />
                    ...
                    <button
                        ...
                        className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 bg-stone-500 rounded-lg hover:bg-stone-400 disabled:opacity-50 transition text-slate-950"
                    >
                        {status === 'loading' ? (
                            ...
                            {filteredSuggestions.map((s) => (
                                <li key={s.slug}>
                                    <button
                                        ...
                                        className="w-full text-left px-4 py-3 text-sm text-slate-700 hover:bg-stone-50 hover:text-stone-700 transition flex items-center justify-between group"
                                    >
                                        <span className="font-medium text-slate-800">{s.name}</span>
                                        <ArrowRight size={14} className="opacity-0 group-hover:opacity-100 transition-opacity text-stone-500" />
                                    </button>
                                </li>
                            ))}
                        </ul>
                ...
                <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4 animate-in fade-in slide-in-from-top-2">
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <Link
                                href="/#simulateur"
                                className="text-xs font-bold text-stone-400 hover:text-stone-300 underline decoration-stone-400/30"
                            >
