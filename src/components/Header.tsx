                ...
                <div className="flex items-center gap-4">
                    ...
                    {isHub && (
                        <div className={`hidden md:flex items-center gap-6 text-sm font-medium ${variant === "light" ? "text-slate-300" : "text-slate-600"}`}>
                            <Link href="/guides" className="hover:text-stone-600 transition">Guías e Ideas</Link>
                            <Link href="#simulateur" className="hover:text-stone-600 transition">Presupuesto a Medida</Link>
                        </div>
                    )}

                    {/* RGE Badge (Desktop) */}
                    <div className="hidden lg:flex items-center gap-2 bg-stone-500/10 border border-stone-500/20 px-3 py-1.5 rounded-full">
                        <span className="w-2 h-2 rounded-full bg-stone-500 animate-pulse"></span>
                        <span className="text-xs font-bold text-stone-700">Instaladores Premium de Pérgolas</span>
                    </div>
