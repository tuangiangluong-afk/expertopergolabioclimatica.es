...
export default async function BlogPost({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const schema = {
        ...
        "publisher": {
            ...
            "logo": {
                ...
                "url": "https://www.expertopergolabioclimatica.es/logo.png"
        ...
        "mainEntityOfPage": {
            ...
            "@id": "https://www.expertopergolabioclimatica.es/blog/"
    ...
    return (
        <main className="min-h-screen bg-white text-neutral-900 font-sans selection:bg-stone-100 selection:text-stone-900 pt-20">
             <script
            ...
            <nav className="container mx-auto px-4 py-4 border-b border-neutral-100">
                <ol className="flex items-center space-x-2 text-sm text-neutral-500">
                    <li><Link href="/" className="hover:text-stone-600 transition-colors">Accueil</Link></li>
                    <li><ChevronRight className="w-3 h-3" /></li>
                    <li><Link href="/blog" className="hover:text-stone-600 transition-colors">Blog</Link></li>
                    <li><ChevronRight className="w-3 h-3" /></li>
            ...
            <header className="relative w-full h-[400px] bg-neutral-900 flex items-end">
                ...
                <div className="container mx-auto px-4 pb-12 relative z-10">
                   <div className="max-w-4xl">
                        {post.category && (
                            <span className="inline-block bg-stone-600 text-white text-xs font-bold px-3 py-1 rounded-full mb-4 uppercase tracking-wider">
                                {post.category.name}
                            </span>
                        )}
            ...
            <div className="container mx-auto px-4 py-12">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                    ...
                    <article className="lg:col-span-8">
                          {post.excerpt && (
                            <div className="bg-stone-50 border-l-4 border-stone-600 p-6 rounded-r-xl mb-10 text-lg font-medium text-stone-900">
                                {post.excerpt}
                            </div>
                        )}
                        ...
                        <div 
                            ...
                            prose-a:text-stone-600 prose-a:no-underline hover:prose-a:underline
                                ...
                                <div className="space-y-4">
                                     {post.faq.map((item, index) => (
                                        <details key={index} className="group bg-white rounded-xl border border-neutral-200 overflow-hidden">
                                            <summary className="flex items-center justify-between p-4 cursor-pointer font-semibold hover:text-stone-600">
                                                {item.question}
                    ...
                    <aside className="lg:col-span-4 space-y-8">
                        ...
                        <div className="bg-white border border-neutral-200 rounded-2xl p-6 shadow-sm">
                            ...
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 rounded-full bg-stone-100 flex items-center justify-center text-stone-600 font-bold overflow-hidden relative">
                                    {(post.author_name || 'E')[0]}
                                      {/* Verified Badge */}
                                    <div className="absolute -bottom-1 -right-1 bg-white rounded-full p-0.5 shadow-sm">
                                        <div className="bg-stone-500 rounded-full w-3 h-3 border-2 border-white"></div>
                                    </div>
                                </div>
                                <div>
                         ...
                         {/* CTA */}
                         <div className="bg-gradient-to-br from-stone-950 to-stone-900 text-white rounded-2xl p-8 text-center shadow-lg">
                            <h4 className="text-xl font-bold mb-2">Presupuesto Gratuito</h4>
                            <p className="text-stone-100 text-sm mb-6">Comparez les prix des installateurs RGE près de chez vous.</p>
                            <Link href="/devis" className="inline-block bg-white text-stone-900 px-6 py-3 rounded-full font-bold hover:bg-stone-50 transition-colors w-full">
                                Commencer
                            </Link>
                         </div>
                    </aside>
